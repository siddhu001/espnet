...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def encode(
        ...
        if self.postdecoder is not None:
            ...
            # print(self.transcript_token_list)
            transcript_list=[" ".join([self.transcript_token_list[int(k)] for k in k1 if k!="<sos/eos>"]) for k1 in transcript_pad]
            # print("ok1")
            ...
            input_id_length =self.postdecoder.convert_examples_to_features(transcript_list, 128)
            # print("ok")
            ...
            # print(max_lens)
            encoder_new_out=torch.zeros((encoder_out.shape[0],max_lens,encoder_out.shape[2])).to(device=device)
            for k in range(len(encoder_out)):
                # print(encoder_out[k,:encoder_out_lens[k]].shape)

                encoder_new_out[k]=torch.cat((encoder_out[k,:encoder_out_lens[k]],\
                    bert_encoder_out[k,:bert_encoder_lens[k]],\
                    torch.zeros((max_lens-final_encoder_out_lens[k],encoder_out.shape[2])).to(device=device)),0)
