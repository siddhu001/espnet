...
def extract_last_text_bpe(conversation):
    ...
    for entry in conversation:
        ...
        #     total_entry.append(entry)
        if entry[0] == "assistant" and entry[1] == "text_bpe":
            last_user = entry
        elif entry[0] == "assistant" and entry[1] == "codec_ssl":
            last_assistant = entry
    
    return last_user, last_assistant, conversation[:-2]
...
def process_file(input_file, output_file=None, output_valid_file=None):
    ...
    for base_key, samples in grouped_train.items():
        ...
        if combined:
            ...
            dialogue.add_segment(
                role=combined[0][0],
                modality=combined[0][1],
                target=False,
                content=combined[0][-1],
            )
            dialogue.add_segment(
                role=combined[1][0],
                modality=combined[1][1],
                target=True,
            ...
            dialogue.add_segment(
                role=combined[2][0],
                modality=combined[2][1],
                target=False,
                content=combined[2][-1],
            )
            dialogue.add_segment(
                role=combined[3][0],
                modality=combined[3][1],
                target=True,
                content=combined[3][-1],
            )
            example_id = base_key
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python extract_text_bpe.py <input_file> <output_train_dir> <output_valid_dir>")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_dir = sys.argv[2]
    output_valid_dir = sys.argv[3]
    
    process_file(input_file, output_dir, output_valid_dir)