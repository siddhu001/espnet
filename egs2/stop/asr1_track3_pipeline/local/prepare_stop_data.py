...
dir_dict = {
    "train-full": "manifests/train.tsv",
    "valid-full": "manifests/eval.tsv",
    "test-full": "manifests/test.tsv",
}
...
for x in dir_dict:
    with open(os.path.join("data", x, "text"), "w") as text_f, open(
        ...
        print(len(transcript_df.values))
        # lines = sorted(transcript_df.values, key=lambda s: s[0])
        for row in transcript_df.values:
            if str(row[-1]) == "nan":
                # cannot remove nan utterances from test data
                if x == "test-full":
                    path_arr = row[0].split("/")
                    utt_id = path_arr[-2] + "_" + path_arr[-1]
                    print("add nan", utt_id)
                else:
                    path_arr = row[0].split("/")
                    utt_id = path_arr[-2] + "_" + path_arr[-1]
                    print("skip nan", utt_id)
                    continue
            words = str(row[-1]).lower()
            # print(words)
            path_arr = row[0].split("/")
            utt_id = path_arr[-2] + "_" + path_arr[-1]
            # print(utt_id + " " + words + "\n")
            ...
            text_f.write(utt_id + " " + str(row[-3]).lower() + "\n")
            transcript_f.write(utt_id + " " + words + "\n")
