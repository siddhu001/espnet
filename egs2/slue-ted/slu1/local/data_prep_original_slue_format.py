...
for x in dir_dict:
    with open(os.path.join("data", x, "text"), "w") as text_f, open(
    ...
    ) as wav_scp_f, open(os.path.join("data", x, "utt2spk"), "w") as utt2spk_f, open(os.path.join("data", x, "segments"), "w") as segment_f:
        text_f.truncate()
        wav_scp_f.truncate()
        utt2spk_f.truncate()
        ...
        for row in transcript_df.values:
            ...
            words=row[4].strip().encode("ascii", "ignore").decode().replace("\n"," ").lower()+" [sep] "+row[5].strip().encode("ascii", "ignore").decode().replace("\n"," ").lower()
            ...
            if x=="devel":
                ...
                path=os.path.join(root,"slue-ted_"+x+"/"+x,segmentid+".flac")
