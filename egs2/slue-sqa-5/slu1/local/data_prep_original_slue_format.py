    ...
    with open(os.path.join("data", x, "text"), "w") as text_f, open(
        ...
        for row in transcript_df.values:
            ...
            if len(row[9].split("], ["))>1:
                timestamp_file=open(os.path.join(os.path.join(root, timestamp_dir[x]),row[4]+".txt"))
                ...
                for val in row[9].split("], ["):
                    if val.split('"')[-1].split(", ")[1].strip() not in unique_start_dict:
                        unique_start_dict[val.split(", ")[1].strip()]=1
                    if val.split('"')[-1].split(",")[2].strip().replace("]","") not in unique_end_dict:
                ...
                for line in timestamp_line_arr:
                    ...
                    #     import pdb;pdb.set_trace()
                    if line.split("\t")[2]==row[9].split('"')[-1].split(", ")[1].strip():
                        found_start=True
                    if line.split("\t")[3].strip()==row[9].split('"')[-1].split(", ")[2].strip().replace("]",""):
                        found_end=True
                        found_ever+=1
                    if found_start:
                        doc_sentence+='ANS '
