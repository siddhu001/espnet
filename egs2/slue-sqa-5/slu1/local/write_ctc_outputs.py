...
class ReadCTCEmissions:
        ...
        for idx, item_tuple in enumerate(zip(subtoken_lst, dur_lst)):
            ...
            if subtoken == "ANS":
                ...
                if part_of_entity:
                    end_time += dur
                    # import pdb;pdb.set_trace()
                    tstamp_tuple_lst.append((entity_token.replace("▁"," ").replace("<blank>","").strip(),start_time, end_time))
                    part_of_entity = False
                    entity_token=""
                else:
                    part_of_entity = True
    ...
    def get_char_outputs(self, frame_len):
        ...
        save_json(os.path.join(save_dir, f"{self.split}_pred_answer.json"), res_dct)
