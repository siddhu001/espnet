...
class ReadCTCEmissions:
    ...
    def get_char_outputs(self, frame_len):
        ...
        frame_level_op_ctc = read_lst("/scratch/bbjs/arora1/espnet_slue_PR/espnet/egs2/slue-sqa-5/slu1/exp/slu_train_asr_wav2vec2_raw_en_bpe1000/decode_asr_full_path_slu_model_valid.acc.ave/test/token")
                ...
                if len(res_dct[utt_id])>0:
                    try:
                        ...
                        if ctc_out.count(res_dct[utt_id][0][0][1:-1])==0:
                            if cnt_fill==2:
                                res_dct[utt_id] = self.extract_entity_ctc_timestamps(subtoken_ctc_lst, dur_lst)
                            else:
                                print("ok")
                                res_dct[utt_id]=[]
                        elif ctc_out.count(res_dct[utt_id][0][0][1:-1])==1:
                            # import pdb;pdb.set_trace()
                            add_count=ctc_out[: ctc_out.index(res_dct[utt_id][0][0][1:-1])].count("▁")
                            find_count=0
                            subtoken_ctc_lst1=[]
                            dur_lst1=[]
                            ...
                            cnt_fill = len(np.where(np.array(subtoken_ctc_lst1) == "ANS")[0])
                            if cnt_fill<2:
                                print("oookk")
                                subtoken_ctc_lst1.append("ANS")
                                dur_lst1.append(0.0)
                                ...
                                import pdb;pdb.set_trace()
                            res_dct[utt_id] = self.extract_entity_ctc_timestamps(subtoken_ctc_lst1, dur_lst1)
                            # import pdb;pdb.set_trace()
                        else:
                            # import pdb;pdb.set_trace()
                            ctc_start=[m.start() for m in re.finditer(res_dct[utt_id][0][0][1:-1], ctc_out)]
                            dec_start=[m.start() for m in re.finditer(res_dct[utt_id][0][0][1:-1], ''.join(subtokens))]
                            for dec_index in range(len(dec_start)):
                                if (''.join(subtokens)[:dec_start[dec_index]].count("ANS")==1):
