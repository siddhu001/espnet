...
src_case=lc.rm
src_token_type=bpe      # Tokenization type (char or bpe) for source type.
src_nbpe=30             # The number of BPE vocabulary for source type.
src_bpemode=unigram     # Mode of BPE for source type (unigram or bpe).
src_bpe_input_sentence_size=100000000 # Size of input sentence for BPE for source type.
src_bpe_nlsyms=         # non-linguistic symbols list, separated by a comma, for BPE of source type
src_bpe_char_cover=1.0  # character coverage when modeling BPE for source type
tgt_case=tc
tgt_token_type=bpe      # Tokenization type (char or bpe) for target type.
tgt_nbpe=30             # The number of BPE vocabulary for target type.
tgt_bpemode=unigram     # Mode of BPE (unigram or bpe) for target type.
tgt_bpe_input_sentence_size=100000000 # Size of input sentence for BPE for target type.
tgt_bpe_nlsyms=         # non-linguistic symbols list, separated by a comma, for BPE for target type.
tgt_bpe_char_cover=1.0  # character coverage when modeling BPE for target type.
...
use_lm=false      # Use language model for NLU decoding.
...
# NLU model related
nlu_tag=        # Suffix to the result dir for nlu model training.
nlu_exp=        # Specify the directory path for NLU experiment.
               # If this option is specified, nlu_tag is ignored.
nlu_stats_dir=  # Specify the directory path for NLU statistics.
nlu_config=     # Config for nlu model training.
nlu_args=       # Arguments for nlu model training, e.g., "--max_epoch 10".
               # Note that it will overwrite args in nlu config.
ignore_init_mismatch=false      # Ignore initial mismatch
num_splits_nlu=1            # Number of splitting for lm corpus.
src_type=asr                # source type abbrev. id (e.g., es)
tgt_type=ner                # target type abbrev. id (e.g., en)
...
inference_nlu_model=valid.acc.ave.pth # NLU model path for decoding.
                                      # e.g.
                                      # inference_nlu_model=train.loss.best.pth
                                      # inference_nlu_model=3epoch.pth
                                      # inference_nlu_model=valid.acc.best.pth
                                      # inference_nlu_model=valid.loss.ave.pth
...
src_bpe_train_text=  # Text file path of bpe training set for source type.
tgt_bpe_train_text=  # Text file path of bpe training set for target type.
...
nlu_text_fold_length=150   # fold_length for text data during NLU training.
...
Options:
    ...
    # NLU model related
    --nlu_tag           # Suffix to the result dir for nlu model training (default="${nlu_tag}").
    --nlu_exp           # Specify the directory path for NLU experiment.
                       # If this option is specified, nlu_tag is ignored (default="${nlu_exp}").
    --nlu_stats_dir     # Specify the directory path for NLU statistics (default="${nlu_stats_dir}").
    --nlu_config        # Config for nlu model training (default="${nlu_config}").
    --nlu_args          # Arguments for nlu model training (default="${nlu_args}").
                       # e.g., --nlu_args "--max_epoch 10"
                       # Note that it will overwrite args in nlu config.
    --ignore_init_mismatch=      # Ignore mismatch parameter init with pretrained model (default="${ignore_init_mismatch}").
    --num_splits_nlu    # Number of splitting for lm corpus.  (default="${num_splits_nlu}").
    --src_type=        # source type abbrev. id (e.g., asr). (default="${src_type}")
    --tgt_type=        # target type abbrev. id (e.g., intent). (default="${tgt_type}")
    ...
    --inference_nlu_model # NLU model path for decoding (default="${inference_nlu_model}").
    ...
    --nlu_text_fold_length   # fold_length for text data during NLU training (default="${nlu_text_fold_length}").
...
# Extra files for NLU process
utt_extra_files="text.${src_case}.${src_type} text.${tgt_case}.${tgt_type}"
# Use the same text as NLU for bpe training if not specified.
[ -z "${src_bpe_train_text}" ] && src_bpe_train_text="${data_feats}/${train_set}/text.${src_case}.${src_type}"
[ -z "${tgt_bpe_train_text}" ] && tgt_bpe_train_text="${data_feats}/${train_set}/text.${tgt_case}.${tgt_type}"
# Use the same text as NLU for lm training if not specified.
[ -z "${lm_train_text}" ] && lm_train_text="${data_feats}/${train_set}/text.${tgt_case}.${tgt_type}"
# Use the same text as NLU for lm training if not specified.
[ -z "${lm_dev_text}" ] && lm_dev_text="${data_feats}/${valid_set}/text.${tgt_case}.${tgt_type}"
# Use the text of the 1st evaldir if lm_test is not specified
[ -z "${lm_test_text}" ] && lm_test_text="${data_feats}/${test_sets%% *}/text.${tgt_case}.${tgt_type}"
...
if [ -z "${nlu_tag}" ]; then
    if [ -n "${nlu_config}" ]; then
        nlu_tag="$(basename "${nlu_config}" .yaml)_${feats_type}"
    else
        nlu_tag="train_${feats_type}"
    fi
    if [ "${lang}" != noinfo ]; then
    ...
    if [ -n "${nlu_args}" ]; then
        nlu_tag+="$(echo "${nlu_args}" | sed -e "s/--/\_/g" -e "s/[ |=/]//g")"
    fi
fi
if [ -z "${lm_tag}" ]; then
    if [ -n "${lm_config}" ]; then
...
if [ -z "${nlu_stats_dir}" ]; then
    if [ "${lang}" != noinfo ]; then
        nlu_stats_dir="${expdir}/nlu_stats_${feats_type}_${lang}_${tgt_token_type}"
    else
        nlu_stats_dir="${expdir}/nlu_stats_${feats_type}_${tgt_token_type}"
    fi
    if [ "${tgt_token_type}" = bpe ]; then
        nlu_stats_dir+="${tgt_nbpe}"
    fi
fi
...
if [ -z "${inference_tag}" ]; then
    ...
    inference_tag+="_nlu_model_$(echo "${inference_nlu_model}" | sed -e "s/\//_/g" -e "s/\.[^.]*$//g")"
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # First generate tgt type
        if [ "${tgt_token_type}" = bpe ]; then
            log "Stage 4a: Generate token_list from ${tgt_bpe_train_text} using BPE for tgt_type"
            ...
            log "Stage 4a: Generate character level token_list from ${tgt_bpe_train_text}  for tgt_type"
            ...
            # 0 is reserved for CTC-blank for NLU and also used as ignore-index in the other task
        ...
        # Then generate src type
        if [ "${src_token_type}" = bpe ]; then
            log "Stage 4b: Generate token_list from ${src_bpe_train_text} using BPE for src_type"
            ...
            log "Stage 4b: Generate character level token_list from ${src_bpe_train_text}  for src_type"
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        _nlu_train_dir="${data_feats}/${train_set}"
        _nlu_valid_dir="${data_feats}/${valid_set}"
        log "Stage 9: NLU collect stats: train_set=${_nlu_train_dir}, valid_set=${_nlu_valid_dir}"

        _opts=
        if [ -n "${nlu_config}" ]; then
            # To generate the config file: e.g.
            #   % python3 -m espnet2.bin.nlu_train --print_config --optim adam
            _opts+="--config ${nlu_config} "
        fi

        # 1. Split the key file
        _logdir="${nlu_stats_dir}/logdir"
        mkdir -p "${_logdir}"

        _scp=text.${src_case}.${src_type}

        # Get the minimum number among ${nj} and the number lines of input files
        _nj=$(min "${nj}" "$(<${_nlu_train_dir}/${_scp} wc -l)" "$(<${_nlu_valid_dir}/${_scp} wc -l)")

        key_file="${_nlu_train_dir}/${_scp}"
        split_scps=""
        for n in $(seq "${_nj}"); do
            split_scps+=" ${_logdir}/train.${n}.scp"
        done
        ...
        # 2. Generate run.sh
        log "Generate '${nlu_stats_dir}/run.sh'. You can resume the process from stage 9 using this script"
        mkdir -p "${nlu_stats_dir}"; echo "${run_args} --stage 9 \"\$@\"; exit \$?" > "${nlu_stats_dir}/run.sh"; chmod +x "${nlu_stats_dir}/run.sh"

        # 3. Submit jobs
        log "NLU collect-stats started... log: '${_logdir}/stats.*.log'"
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.nlu_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${tgt_bpemodel}" \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --train_data_path_and_name_and_type "${_nlu_train_dir}/text.${tgt_case}.${tgt_type},text,text" \
                --train_data_path_and_name_and_type "${_nlu_train_dir}/text.${src_case}.${src_type},src_text,text" \
                --valid_data_path_and_name_and_type "${_nlu_valid_dir}/text.${tgt_case}.${tgt_type},text,text" \
                --valid_data_path_and_name_and_type "${_nlu_valid_dir}/text.${src_case}.${src_type},src_text,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
                --valid_shape_file "${_logdir}/valid.JOB.scp" \
                --output_dir "${_logdir}/stats.JOB" \
                ${_opts} ${nlu_args} || { cat "${_logdir}"/stats.1.log; exit 1; }
        ...
        # shellcheck disable=SC2086
        ${python} -m espnet2.bin.aggregate_stats_dirs ${_opts} --output_dir "${nlu_stats_dir}"

        # Append the num-tokens at the last dimensions. This is used for batch-bins count
        <"${nlu_stats_dir}/train/text_shape" \
            awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
            >"${nlu_stats_dir}/train/text_shape.${tgt_token_type}"

        <"${nlu_stats_dir}/train/src_text_shape" \
            awk -v N="$(<${src_token_list} wc -l)" '{ print $0 "," N }' \
            >"${nlu_stats_dir}/train/src_text_shape.${src_token_type}"

        <"${nlu_stats_dir}/valid/text_shape" \
            awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
            >"${nlu_stats_dir}/valid/text_shape.${tgt_token_type}"

        <"${nlu_stats_dir}/valid/src_text_shape" \
            awk -v N="$(<${src_token_list} wc -l)" '{ print $0 "," N }' \
            >"${nlu_stats_dir}/valid/src_text_shape.${src_token_type}"
    fi

    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        _nlu_train_dir="${data_feats}/${train_set}"
        _nlu_valid_dir="${data_feats}/${valid_set}"
        log "Stage 10: NLU Training: train_set=${_nlu_train_dir}, valid_set=${_nlu_valid_dir}"
        ...
        if [ "${num_splits_nlu}" -gt 1 ]; then
            # If you met a memory error when parsing text files, this option may help you.
            # The corpus is split into subsets and each subset is used for training one by one in order,
            # so the memory footprint can be limited to the memory required for each dataset.

            _split_dir="${nlu_stats_dir}/splits${num_splits_nlu}"
            if [ ! -f "${_split_dir}/.done" ]; then
                rm -f "${_split_dir}/.done"
                ${python} -m espnet2.bin.split_scps \
                  --scps \
                      "${_nlu_train_dir}/${_scp}" \
                      "${_nlu_train_dir}/text.${tgt_case}.${tgt_type}" \
                      "${_nlu_train_dir}/text.${src_case}.${src_type}" \
                      "${nlu_stats_dir}/train/text_shape.${tgt_token_type}" \
                      "${nlu_stats_dir}/train/src_text_shape.${src_token_type}" \
                  --num_splits "${num_splits_nlu}" \
                  --output_dir "${_split_dir}"
                touch "${_split_dir}/.done"
            else
                log "${_split_dir}/.done exists. Spliting is skipped"
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.${tgt_case}.${tgt_type},text,text "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.${src_case}.${src_type},src_text,text "
            ...
            _opts+="--train_data_path_and_name_and_type ${_nlu_train_dir}/text.${tgt_case}.${tgt_type},text,text "
            _opts+="--train_data_path_and_name_and_type ${_nlu_train_dir}/text.${src_case}.${src_type},src_text,text "
            _opts+="--train_shape_file ${nlu_stats_dir}/train/text_shape.${tgt_token_type} "
            _opts+="--train_shape_file ${nlu_stats_dir}/train/src_text_shape.${src_token_type} "
        fi

        log "Generate '${nlu_exp}/run.sh'. You can resume the process from stage 10 using this script"
        mkdir -p "${nlu_exp}"; echo "${run_args} --stage 10 \"\$@\"; exit \$?" > "${nlu_exp}/run.sh"; chmod +x "${nlu_exp}/run.sh"

        # NOTE(kamo): --fold_length is used only if --batch_type=folded and it's ignored in the other case
        log "NLU training started... log: '${nlu_exp}/train.log'"
        if echo "${cuda_cmd}" | grep -e queue.pl -e queue-freegpu.pl &> /dev/null; then
            # SGE can't include "/" in a job name
            jobname="$(basename ${nlu_exp})"
        else
            jobname="${nlu_exp}/train.log"
        fi
        ...
        ${python} -m espnet2.bin.launch \
            --cmd "${cuda_cmd} --name ${jobname}" \
            --log "${nlu_exp}"/train.log \
            --ngpu "${ngpu}" \
            --num_nodes "${num_nodes}" \
            --init_file_prefix "${nlu_exp}"/.dinlu_init_ \
            --multiprocessing_distributed true -- \
            ${python} -m espnet2.bin.nlu_train \
                --use_preprocessor true \
                --bpemodel "${tgt_bpemodel}" \
                ...
                --valid_data_path_and_name_and_type "${_nlu_valid_dir}/text.${tgt_case}.${tgt_type},text,text" \
                --valid_data_path_and_name_and_type "${_nlu_valid_dir}/text.${src_case}.${src_type},src_text,text" \
                --valid_shape_file "${nlu_stats_dir}/valid/text_shape.${tgt_token_type}" \
                --valid_shape_file "${nlu_stats_dir}/valid/src_text_shape.${src_token_type}" \
                --resume true \
                --ignore_init_mismatch ${ignore_init_mismatch} \
                --fold_length "${nlu_text_fold_length}" \
                --fold_length "${nlu_text_fold_length}" \
                --output_dir "${nlu_exp}" \
                ${_opts} ${nlu_args}
...
if [ -n "${download_model}" ]; then
    ...
    nlu_exp="${expdir}/${download_model}"
    mkdir -p "${nlu_exp}"

    # If the model already exists, you can skip downloading
    espnet_model_zoo_download --unpack true "${download_model}" > "${nlu_exp}/config.txt"

    # Get the path of each file
    _nlu_model_file=$(<"${nlu_exp}/config.txt" sed -e "s/.*'nlu_model_file': '\([^']*\)'.*$/\1/")
    _nlu_train_config=$(<"${nlu_exp}/config.txt" sed -e "s/.*'nlu_train_config': '\([^']*\)'.*$/\1/")

    # Create symbolic links
    ln -sf "${_nlu_model_file}" "${nlu_exp}"
    ln -sf "${_nlu_train_config}" "${nlu_exp}"
    inference_nlu_model=$(basename "${_nlu_model_file}")

    if [ "$(<${nlu_exp}/config.txt grep -c lm_file)" -gt 0 ]; then
        _lm_file=$(<"${nlu_exp}/config.txt" sed -e "s/.*'lm_file': '\([^']*\)'.*$/\1/")
        _lm_train_config=$(<"${nlu_exp}/config.txt" sed -e "s/.*'lm_train_config': '\([^']*\)'.*$/\1/")
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        log "Stage 11: Decoding: training_dir=${nlu_exp}"
        ...
        # 2. Generate run.sh
        log "Generate '${nlu_exp}/${inference_tag}/run.sh'. You can resume the process from stage 11 using this script"
        mkdir -p "${nlu_exp}/${inference_tag}"; echo "${run_args} --stage 11 \"\$@\"; exit \$?" > "${nlu_exp}/${inference_tag}/run.sh"; chmod +x "${nlu_exp}/${inference_tag}/run.sh"
        ...
        for dset in ${test_sets}; do
            ...
            nlu_inference_tool="espnet2.bin.nlu_inference"
            ...
            log "Decoding started... log: '${_logdir}/nlu_inference.*.log'"
            # shellcheck disable=SC2086
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/nlu_inference.JOB.log \
                ${python} -m ${nlu_inference_tool} \
                    --batch_size ${batch_size} \
                    --ngpu "${_ngpu}" \
                    --data_path_and_name_and_type "${_data}/${_scp},src_text,text" \
                    --key_file "${_logdir}"/keys.JOB.scp \
                    --nlu_train_config "${nlu_exp}"/config.yaml \
                    --nlu_model_file "${nlu_exp}"/"${inference_nlu_model}" \
...
packed_model="${nlu_exp}/${nlu_exp##*/}_${inference_nlu_model%.*}.zip"
if ! "${skip_upload}"; then
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        # shellcheck disable=SC2086
        ${python} -m espnet2.bin.pack nlu \
            --nlu_train_config "${nlu_exp}"/config.yaml \
            --nlu_model_file "${nlu_exp}"/"${inference_nlu_model}" \
            ${_opts} \
            --option "${nlu_exp}"/RESULTS.md \
            --option "${nlu_exp}"/RESULTS.md \
            --option "${nlu_exp}"/images \
...
<li><strong>Results</strong><pre><code>$(cat "${nlu_exp}"/RESULTS.md)</code></pre></li>
<li><strong>NLU config</strong><pre><code>$(cat "${nlu_exp}"/config.yaml)</code></pre></li>
...
EOF
        ...
        espnet_model_zoo_upload \
            ...
            --description_file "${nlu_exp}"/description \
...
if ! "${skip_upload_hf}"; then
    if [ ${stage} -le 15 ] && [ ${stop_stage} -ge 15 ]; then
        ...
        # Generate description file
        # shellcheck disable=SC2034
        hf_task=natural-language-understanding
        # shellcheck disable=SC2034     
        espnet_task=NLU
        # shellcheck disable=SC2034
        task_exp=${nlu_exp}
