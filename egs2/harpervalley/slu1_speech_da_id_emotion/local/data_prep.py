...
def process_data(target_dir, source_dir, audio_dir, filename):
    ...
    with open(
        ...
        for v in transcript_f:
            ...
            emotion_dict = v["emotion"]
            inverse = [(value, key) for key, value in emotion_dict.items()]
            emotion_label = (max(inverse)[1])
            nolex_arr=["[noise]","[laughter]","<unk>","[kids]","[baby]","[music]","[cough]","[unintelligible]","[dogs]","[ringing]"]
            for nolex in nolex_arr:
                transcript=transcript.replace(nolex,"")
...
# print(args.filename)
process_data(
    args.target_dir, args.source_dir, args.audio_dir, args.filename
)
